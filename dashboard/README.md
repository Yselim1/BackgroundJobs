...
The dashboard uses clean History API routes. Production hosting must serve `index.html` for unknown frontend paths such as `/jobs` and `/logs/:executionId`, while continuing to proxy `/api` and `/health` to the backend.
