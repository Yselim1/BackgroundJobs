...
import { applyBackfill, getExecutions, getJobPlan, previewBackfill } from '../api';
...
export function JobDetailPage(props: JobDetailPageProps) {
    ...
    return <>
        <section className="job-detail-page">
            <div className="page-heading job-detail-heading"><div><p className="eyebrow">Job definition · version {job.version}</p><h1>{job.name}</h1><p><code>{job.id}</code>{job.description === undefined ? '' : ' · ' + job.description}</p></div><div className="page-actions"><a className="button button-quiet" href="/jobs">Back to jobs</a><a className="button button-quiet" href={'/logs?jobId=' + encodeURIComponent(job.id)}>View logs</a>{props.canWrite && <a className="button button-quiet" href={'/jobs?edit=' + encodeURIComponent(job.id)}>Edit job</a>}{job.schedule !== undefined && props.canRun && <button className="button button-quiet" onClick={() => setBackfillOpen(true)}>Backfill</button>}<button className="button button-run" disabled={!props.canRun || props.runBusy === 'run:' + job.id} onClick={() => void props.onRun(job.id)}>Run now</button></div></div>
            {backfillOpen && <BackfillPanel job={job} onClose={() => setBackfillOpen(false)} onError={props.onError} />}
            ...
            {tab === 'automations' && <JobAutomationsPanel job={job} jobs={props.jobs} canWrite={props.canWrite} refreshVersion={props.liveVersion} onOpenExecution={props.onOpenExecution} onError={props.onError} />}
        </section>
    </>;
}
...
function BackfillPanel(props: { job: Job; onClose: () => void; onError: (message?: string) => void }) {
    const now = new Date();
    const [from, setFrom] = useState(new Date(now.getTime() - 86_400_000).toISOString().slice(0, 16));
    const [to, setTo] = useState(now.toISOString().slice(0, 16));
    const [input, setInput] = useState('');
    const [occurrences, setOccurrences] = useState<Array<{ scheduledFor: string; existingExecutionId: string | null }>>([]);
    const [busy, setBusy] = useState(false);
    const dates = () => ({ from: new Date(from).toISOString(), to: new Date(to).toISOString() });
    const preview = async () => { setBusy(true); try { const range = dates(); setOccurrences((await previewBackfill(props.job.id, range.from, range.to)).occurrences); props.onError(undefined); } catch (error) { props.onError(error instanceof Error ? error.message : String(error)); } finally { setBusy(false); } };
    const apply = async () => {
        if (!window.confirm(`Queue ${occurrences.filter(item => item.existingExecutionId === null).length} missing occurrences?`)) return;
        setBusy(true);
        try { const range = dates(); const parsed = input.trim() ? JSON.parse(input) as Record<string, unknown> : undefined; await applyBackfill(props.job.id, range.from, range.to, parsed, crypto.randomUUID()); props.onClose(); }
        catch (error) { props.onError(error instanceof Error ? error.message : String(error)); }
        finally { setBusy(false); }
    };
    return <section className="panel backfill-panel"><div className="panel-heading"><div><p className="eyebrow">Backfill preview</p><h2>Recompute scheduled occurrences</h2></div><button className="close" onClick={props.onClose}>×</button></div><div className="backfill-fields"><label>From<input type="datetime-local" value={from} onChange={event => setFrom(event.target.value)} /></label><label>To<input type="datetime-local" value={to} onChange={event => setTo(event.target.value)} /></label><label>Optional input JSON<textarea className="code-input" value={input} onChange={event => setInput(event.target.value)} placeholder="Use DEFAULT_INPUT" /></label></div><button className="button button-quiet" disabled={busy} onClick={() => void preview()}>Preview</button>{occurrences.length > 0 && <div className="backfill-occurrences">{occurrences.map(item => <span key={item.scheduledFor} className={item.existingExecutionId ? 'existing' : ''}>{new Date(item.scheduledFor).toLocaleString()} {item.existingExecutionId ? '· existing' : '· new'}</span>)}</div>}<button className="button button-primary" disabled={busy || occurrences.length === 0} onClick={() => void apply()}>Apply backfill</button></section>;
}
